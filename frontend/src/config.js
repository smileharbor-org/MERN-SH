export const VITE_FACEBOOK = import.meta.env.VITE_FACEBOOK 
export const VITE_TWITTER = import.meta.env.VITE_TWITTER
export const VITE_YOUTUBE = import.meta.env.VITE_YOUTUBE
export const VITE_LINKEDIN = import.meta.env.VITE_LINKEDIN
export const VITE_GOOGLE_SHEETS_API  = import.meta.env.VITE_GOOGLE_SHEETS_API 
export const VITE_RAZORPAY_KEY  = import.meta.env.VITE_RAZORPAY_KEY 
export const VITE_RAZORPAT_SECRET  = import.meta.env.VITE_RAZORPAT_SECRET 
export const VITE_GOOGLE_DONATION  = import.meta.env.VITE_GOOGLE_DONATION
export const VITE_EMAILJS_TEMPLATE  = import.meta.env.VITE_EMAILJS_TEMPLATE
export const VITE_SERVICE_ID  = import.meta.env.VITE_SERVICE_ID
export const VITE_PUBLIC_KEY  = import.meta.env.VITE_PUBLIC_KEY
export const VITE_BACKEND_URL  = import.meta.env.VITE_BACKEND_URL


